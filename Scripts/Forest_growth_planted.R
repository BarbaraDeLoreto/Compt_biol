...
# initial planted forest in rows: 
...
simulateForest <- function(iniForest, tmax, p) {
  ...
  for(i in 1:tmax) {
    ...
    # Updating the forest state, removing dead trees (immediately)
    ...
    for(j in alive) {
      if ((ftt[[i + 1]]$r[j]>= p$maturityr) && (runif(1) < p$lambda)){ #&& (is.na(simResult[[49 + 1]]$ID) == T))  # if r is at maturity or larger, reproduction happening?
        ...
        attempt <- 0
        max_attempts <- 100 #maximum number of placement attempts
        ...
        repeat{  
          new_x <- runif(1, ftt[[i + 1]]$x[j] - p$d_d, ftt[[i + 1]]$x[j] + p$d_d)
...
saveGIF(forestMovie(simResult_planted, p), movie.name = "C:/Users/badel/Documents/Assignment_Compt_biol_2/Images/planted_rows.gif")



# Plotting number of trees when density is achieved
data_p <- data.frame(time = 1:tmax, n_trees = NA)

for (i in 1:tmax) {
  data_p$n_trees[i] <- sum(simResult_planted[[i + 1]]$alive)
}

# Call the pdf command to start the plot
pdf(file = "../Plots/Desity_planted_rows.pdf",   
    width = 4, # The width of the plot in inches
    height = 4) # The height of the plot in inches

#density plot
ggplot(data_p, aes(time, n_trees))+
  geom_line() +
...
##Planted forest random
# running the simulation for planted_forest:
simResult_planted_r <- simulateForest(planted_forest_random, tmax, p)
saveGIF(forestMovie(simResult_planted, p), movie.name = "C:/Users/badel/Documents/Assignment_Compt_biol_2/Images/planted_random.gif")
...
for (i in 1:tmax) {
  data_p$n_trees[i] <- sum(simResult_planted_r[[i + 1]]$alive)
}

# Call the pdf command to start the plot
pdf(file = "../Plots/Desity_planted_random.pdf",   
    width = 4, # The width of the plot in inches
    height = 4) # The height of the plot in inches
