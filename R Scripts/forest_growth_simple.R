...
simulateForest <- function(iniForest, tmax, p) {
  ...
  for(i in 1:tmax) {
    ...
    ftt[[i + 1]]$r[canGrow] <- ftt[[i + 1]]$r[canGrow] + p$gamma
    # make sure they're not growing beyond maxr:
    ftt[[i + 1]]$r[ftt[[i + 1]]$r > p$maxr] <- p$maxr
    # death:
    ftt[[i + 1]]$alive[alive] <- (runif(length(alive)) > p$mu)
    # reproduction:
    emptySlot <- which(is.na(ftt[[i + 1]]$ID))[1] # next empty row in dataframe
    for(j in alive) {
      if (identical(ftt[[i + 1]]$r[j], p$maxr) && (runif(1) < p$lambda)) { # reproduction happening?
        ftt[[i + 1]]$ID[emptySlot] <- nextID
        ftt[[i + 1]]$alive[emptySlot] <- TRUE
        ftt[[i + 1]]$x[emptySlot] <- runif(1, min = 0, max = p$maxx)
        ftt[[i + 1]]$y[emptySlot] <- runif(1, min = 0, max = p$maxy)
...
p$maxx <- 200      # x-size of forest
p$maxy <- 200      # y-size of forest

p$maxr   <- 10      # maximum radius of a tree
p$gamma  <- 0.2     # growth per time step
p$mu     <- 0.02    # mortality probability per time step
p$lambda <- 0.1     # probability of producing surviving offspring after reaching maturity, per time step

tmax <- 500 # number of time steps to simulate
...
iniForest <- data.frame(ID    = 1:nini,                             # ID for each tree 
                        ...
                        a     = NA)                                 # age
iniForest$a <- iniForest$r/p$gamma # expected age based on their size

# running the simulation:
simResult <- simulateForest(iniForest, tmax, p)
saveGIF(forestMovie(simResult, p), movie.name = "first_forest_movie.gif")